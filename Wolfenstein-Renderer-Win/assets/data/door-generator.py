...
def is_floor_at(grid, x, y):
    return grid[y][x] == ' '

def can_place_run_on_wall(grid, run, door_char):
    for x, y in run:
        if not in_bounds(x, y):
            return False
        if grid[y][x] != '#':
            return False
        if grid[y][x] == door_char:
            return False
    return True

def place_run(grid, run, door_char):
    for x, y in run:
        grid[y][x] = door_char

def add_room_boundary_doors(grid, door_count, door_width=3, min_sep=10):
    half = door_width // 2
    candidates = []
    ...
    for rx, ry, rw, rh in rooms:
        ...
        for y in range(top, bottom + 1):
            wx = left - 1
            if in_bounds(wx, y) and is_wall(grid, wx, y) and in_bounds(wx - 1, y) and is_floor_at(grid, wx - 1, y):
                run = [(wx, yy) for yy in range(y - half, y + half + 1)]
                candidates.append((wx, y, run))
            wx = right + 1
            if in_bounds(wx, y) and is_wall(grid, wx, y) and in_bounds(wx + 1, y) and is_floor_at(grid, wx + 1, y):
                run = [(wx, yy) for yy in range(y - half, y + half + 1)]
                candidates.append((wx, y, run))
        ...
        for x in range(left, right + 1):
            wy = top - 1
            if in_bounds(x, wy) and is_wall(grid, x, wy) and in_bounds(x, wy - 1) and is_floor_at(grid, x, wy - 1):
                run = [(xx, wy) for xx in range(x - half, x + half + 1)]
                candidates.append((x, wy, run))
            wy = bottom + 1
            if in_bounds(x, wy) and is_wall(grid, x, wy) and in_bounds(x, wy + 1) and is_floor_at(grid, x, wy + 1):
                run = [(xx, wy) for xx in range(x - half, x + half + 1)]
                candidates.append((x, wy, run))
    ...
    for cx, cy, run in candidates:
        if len(placed) >= door_count:
            break
        too_close = False
        for px, py in placed:
            if (cx - px) * (cx - px) + (cy - py) * (cy - py) < min_sep * min_sep:
                too_close = True
                break
        if too_close:
            continue

        door_char = str(next_id % 10)

        if not can_place_run_on_wall(grid, run, door_char):
            continue

        place_run(grid, run, door_char)
        placed.append((cx, cy))
        next_id += 1
...
doors_to_place = 4
add_room_boundary_doors(map_grid, doors_to_place, door_width=3, min_sep=10)
